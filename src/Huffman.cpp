...
auto FileParser::HuffmanTable::decode(const uint16_t word) const -> std::pair<uint8_t, uint8_t> {
    const auto& decoding = (*m_table)[static_cast<uint8_t>(word >> 8 & 0xFF)];
...
void FileParser::HuffmanTable::generateLookupTable(const std::vector<HuffmanEncoding>& encodings) {
    m_table = std::make_unique<std::array<HuffmanTableEntry, 256>>();
    for (const auto& encoding : encodings) {
        if (encoding.bitLength <= 8) {
            ...
            for (uint8_t i = 0; i < static_cast<uint8_t>(1 << (8 - encoding.bitLength)); i++) {
                ...
                (*m_table)[index] = HuffmanTableEntry(encoding.bitLength, encoding.value, nullptr);
            }
        } else {
            ...
            if ((*m_table)[partOne].nestedTable == nullptr) {
                (*m_table)[partOne].nestedTable = std::make_unique<std::array<HuffmanTableEntry, 256>>();
            }
            const auto partTwoTable = (*m_table)[partOne].nestedTable.get();
